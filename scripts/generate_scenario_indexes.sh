...
EOF
    ...
    <div class="section">
        <h3>📊 Concentration Maps</h3>
        <p style="color: #666; font-size: 0.9rem; margin-bottom: 1rem;">
            Explore the spatial distribution of pathogens. Colored nodes and segments represent concentration levels.
        </p>
        <ul class="file-list">
EOF

    # List Concentration Maps
    if [[ -f "${scenario_dir}/concentration_map.html" ]]; then
        cat >> "${scenario_dir}/index.html" << EOF
            <li>
                <a href="concentration_map.html" class="file-link">
                    <span class="file-icon">🧪</span>
                    <div class="file-info">
                        <div class="file-name">Concentration Node Map</div>
                        <p class="file-desc">Interactive map of pathogen levels at each network node</p>
                    </div>
                </a>
            </li>
EOF
    ...
    if [[ -f "${scenario_dir}/concentration_segments_map.html" ]]; then
        cat >> "${scenario_dir}/index.html" << EOF
            <li>
                <a href="concentration_segments_map.html" class="file-link">
