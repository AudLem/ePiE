...
gm_path <- testthat::test_path("golden_master", "gm_bega_network_v1.26.rds")
skip_if_not(file.exists(gm_path), "Bega golden master file not found")

gm <- readRDS(gm_path)

test_that("Bega network topology matches golden master", {
  ...
  skip_if_not(dir.exists(file.path(data_root, "basins", "bega")), "Bega basin data not found")
  
  test_output_dir <- tempfile(pattern = "bega_gm_test_")
...
test_that("Bega network point IDs match golden master", {
  repo_root <- rprojroot::find_root(criterion = rprojroot::is_git_root)
  data_root <- file.path(repo_root, "Inputs")
  output_root <- file.path(repo_root, "Outputs")
  ...
  test_output_dir <- tempfile(pattern = "bega_gm_ids_")
  on.exit(unlink(test_output_dir, recursive = TRUE), add = TRUE)
...
test_that("Bega network point coordinates match golden master", {
  repo_root <- rprojroot::find_root(criterion = rprojroot::is_git_root)
  data_root <- file.path(repo_root, "Inputs")
  output_root <- file.path(repo_root, "Outputs")
  ...
  test_output_dir <- tempfile(pattern = "bega_gm_coords_")
  on.exit(unlink(test_output_dir, recursive = TRUE), add = TRUE)
...
test_that("Bega lake connectivity matches golden master", {
  repo_root <- rprojroot::find_root(criterion = rprojroot::is_git_root)
  data_root <- file.path(repo_root, "Inputs")
  output_root <- file.path(repo_root, "Outputs")
  ...
  test_output_dir <- tempfile(pattern = "bega_gm_lake_")
  on.exit(unlink(test_output_dir, recursive = TRUE), add = TRUE)
...
test_that("Bega network topology is consistent with golden master", {
  repo_root <- rprojroot::find_root(criterion = rprojroot::is_git_root)
  data_root <- file.path(repo_root, "Inputs")
  output_root <- file.path(repo_root, "Outputs")
  ...
  test_output_dir <- tempfile(pattern = "bega_gm_topo_")
  on.exit(unlink(test_output_dir, recursive = TRUE), add = TRUE)
