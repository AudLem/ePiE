...
RenderLeafletConcentrationSegmentMap <- function(spec, plots_dir) {
  if (is.null(spec) ||
      is.null(spec$concentration_segments_plot) ||
      nrow(spec$concentration_segments_plot) == 0) {
    return(invisible(NULL))
  }
  ...
  if (!is.null(spec$rivers) && nrow(spec$rivers) > 0) {
    map_widget <- map_widget |>
      leaflet::addPolylines(
        data = spec$rivers,
        color = style$colors$river,
        weight = style$line_widths$fallback_river,
        opacity = 0.25,
        group = "Rivers"
      )
  }
  ...
  if (!is.null(spec$canals) && nrow(spec$canals) > 0) {
    map_widget <- map_widget |>
      leaflet::addPolylines(
        data = spec$canals,
        color = style$colors$canal,
        weight = style$line_widths$fallback_canal,
        opacity = 0.35,
        group = "Canals"
      )
  }
  ...
  map_widget <- map_widget |>
    leaflet::addPolylines(
      data = spec$concentration_segments_plot,
      color = ~color_palette(C_w_map),
      weight = ~segment_weight,
      opacity = 0.9,
      popup = ~popup_html,
      group = "Concentration Segments"
    )
  ...
  tag_title <- htmltools::tags$div(
    htmltools::HTML(paste0(spec$map_title_html, "<b>Display:</b> colored transport segments")),
    style = "background: white; padding: 8px 12px; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.2); font-size: 14px;"
  )
  ...
  map_filename <- if (!is.null(spec$segment_map_filename) && nzchar(spec$segment_map_filename)) {
    spec$segment_map_filename
  } else {
    "concentration_segments_map.html"
  }
  map_html <- file.path(plots_dir, map_filename)
  message("Saving interactive segment map HTML: ", map_html)
  map_libdir <- paste0(tools::file_path_sans_ext(basename(map_html)), "_files")
  unlink(file.path(plots_dir, map_libdir), recursive = TRUE, force = TRUE)
  tryCatch(
    htmlwidgets::saveWidget(map_widget, file = map_html, selfcontained = FALSE, libdir = map_libdir),
    error = function(e) message("Note: interactive segment map save failed: ", e$message)
  )

  if (isTRUE(spec$write_legacy_map)) {
    legacy_html <- file.path(plots_dir, "concentration_segments_map.html")
    if (!identical(normalizePath(map_html, mustWork = FALSE), normalizePath(legacy_html, mustWork = FALSE))) {
      legacy_libdir <- "concentration_segments_map_files"
      unlink(file.path(plots_dir, legacy_libdir), recursive = TRUE, force = TRUE)
      tryCatch(
        htmlwidgets::saveWidget(map_widget, file = legacy_html, selfcontained = FALSE, libdir = legacy_libdir),
        error = function(e) message("Note: legacy interactive segment map save failed: ", e$message)
      )
    }
  }
