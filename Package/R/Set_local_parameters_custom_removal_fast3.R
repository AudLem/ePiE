...
#             Must contain: Pt_type, T_AIR, pH, Wind, H, f_STP,
#             rptMStateK, uwwLoadEnt, uwwCapacit, uwwPrimary, uwwSeconda,
#             f_direct, ID, ID_nxt, basin_id, Down_type, Freq, HL_ID_new
#             NOTE: column 'f_direct' (lowercase) is expected here.
#             Historical versions used 'F_direct' (uppercase).
#   HL      - data.frame of hydro-lake nodes (may have 0 rows if no lakes)
#   cons    - data.frame of per-country consumption values.
#             Column naming is inconsistent across data sources: some use 'cnt',
#             others 'country'. The code normalises 'country' -> 'cnt'.
#   chem    - data.frame of substance properties (one row per API).
#   chem_ii - integer index into chem identifying which substance to process
#
# Return:
#   list(pts, hl) — updated pts and HL data.frames with all local
#   chemical parameters attached as new columns
#
# Reference: Keller et al. (2014), version 3 of the ePiE model.
# The "custom_removal" variant allows overriding SimpleTreat with user-specified
# primary/secondary/tertiary removal fractions.
# TODO(MULTI-PATHOGEN): This function is chemical-specific. Pathogen decay
#   parameters are set by AssignPathogenDecayParameters() in 02_PathogenModel.R.
# ==============================================================================
...
Set_local_parameters_custom_removal_fast3 = function(pts,HL,cons,chem,chem_ii){
  ...
  # BUG NOTE: Some datasets use 'country', code expects 'cnt'
