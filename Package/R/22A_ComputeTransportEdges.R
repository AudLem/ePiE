  ...
  if (!is.null(HL) && nrow(HL) != 0) {
    return(list(
      pts = data.frame(
        ...
        Q_source_id = if ("Q_source_id" %in% names(pts)) pts$Q_source_id else NA_character_,
        Q_reference_short = if ("Q_reference_short" %in% names(pts)) pts$Q_reference_short else NA_character_,
        Q_reference_url = if ("Q_reference_url" %in% names(pts)) pts$Q_reference_url else NA_character_,
        Q_regime = if ("Q_regime" %in% names(pts)) pts$Q_regime else NA_character_,
        Q_data_period = if ("Q_data_period" %in% names(pts)) pts$Q_data_period else NA_character_,
        Q_value_origin = if ("Q_value_origin" %in% names(pts)) pts$Q_value_origin else NA_character_,
        Q_derivation_rule = if ("Q_derivation_rule" %in% names(pts)) pts$Q_derivation_rule else NA_character_,
